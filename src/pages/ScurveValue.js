...
const Scurve = () => {
    ...
    const fetchProgressData = async () => {
      try {
        ...
        const chartData = progressData.map(item => ({
          ...
          planValue: item.plan_value || null,
        ...
        const planValue = chartData.map(item => item.planValue);
        const actualValue = chartData.map(item => item.actualValue);

        setChartData({ reportDates, planProgress, actualProgress, planCost, actualCost, planValue, actualValue, week_no });
        console.log('Data set successfully in state:', { reportDates, planProgress, actualProgress, planCost, actualCost, planValue, actualValue, week_no });
      } catch (error) {
  ...
  const { reportDates, planProgress, actualProgress, planCost, actualCost, planValue, actualValue, week_no } = chartData;
    ...
    <div style={{ height: 'calc(81vh - 0px)', width: "100%" }}>
      ...
      <div style={{ height: 'calc(100vh - 170px)', width: "100%", overflow: "hidden"}}>
        ...
        <LineChart
          ...
          series={[
            { label: 'Planned Volume (M3)', data: planValue, color: 'red' },
            { label: 'Actual Volume (M3)', data: actualValue, color: 'green' },
          ]}
          xAxis={[
            {
              title: 'Report Date',
